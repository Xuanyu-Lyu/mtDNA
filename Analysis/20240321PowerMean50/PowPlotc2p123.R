...
allMeanDiffLL_c2p1 <- c()
for (i in 1:100){
      env <- new.env()
      load(paste0(relativePath,"/c2p1/","loop",i,"/modelSmr.Rdata"), envir = env)
      allMeanDiffLL_c2p1[i] <- env$smr2$Minus2LogLikelihood - env$smr1$Minus2LogLikelihood
}

PN <- ncol(env[["smr1"]][["dataSummary"]][["fam1"]])*length(env[["smr1"]][["dataSummary"]])
meanDiffLL_mtam_c2p1 <- mean(allMeanDiffLL_c2p1)
...
# read all the meanDiffLL of the loops for this condition into a vector
allMeanDiffLL_c2p2 <- c()
for (i in 1:100){
      env <- new.env()
      load(paste0(relativePath,"/c2p2/","loop",i,"/modelSmr.Rdata"), envir = env)
      allMeanDiffLL_c2p2[i] <- env$smr2$Minus2LogLikelihood - env$smr1$Minus2LogLikelihood
}

PN <- ncol(env[["smr1"]][["dataSummary"]][["fam1"]])*length(env[["smr1"]][["dataSummary"]])
meanDiffLL_mtam_c2p2 <- mean(allMeanDiffLL_c2p2)
lamdaUnit_c2p2 <- meanDiffLL_mtam_c2p2/PN
SSize_c2p2 <- 1: n
LamdaVec_c2p2 <- lamdaUnit_c2p2*SSize_c2p2
powVec_c2p2 <- as.numeric(lapply(LamdaVec_c2p2,powerCal, df = 2))
...
allMeanDiffLL_c2p3 <- c()
for (i in 1:100){
      env <- new.env()
      load(paste0(relativePath,"/c2p3/","loop",i,"/modelSmr.Rdata"), envir = env)
      allMeanDiffLL_c2p3[i] <- env$smr2$Minus2LogLikelihood - env$smr1$Minus2LogLikelihood
}

PN <- ncol(env[["smr1"]][["dataSummary"]][["fam1"]])*length(env[["smr1"]][["dataSummary"]])
meanDiffLL_mtam_c2p3 <- mean(allMeanDiffLL_c2p3)
lamdaUnit_c2p3 <- meanDiffLL_mtam_c2p3/PN
SSize_c2p3 <- 1: n
LamdaVec_c2p3 <- lamdaUnit_c2p3*SSize_c2p3
powVec_c2p3 <- as.numeric(lapply(LamdaVec_c2p3,powerCal, df = 2))
...
df_c2 <- rbind(df_c2p1, df_c2p2, df_c2p3
               #,df_c2p4,df_c2p5
               )
...
ggsave( paste0(relativePath_save,"/graph_c2p123.png"), g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
