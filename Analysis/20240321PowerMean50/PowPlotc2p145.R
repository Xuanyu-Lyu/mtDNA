...
# read all the meanDiffLL of the loops for this condition into a vector
allMeanDiffLL_c2p4 <- c()
for (i in 1:100){
      env <- new.env()
      load(paste0(relativePath,"/c2p4/","loop",i,"/modelSmr.Rdata"), envir = env)
      allMeanDiffLL_c2p4[i] <- env$smr2$Minus2LogLikelihood - env$smr1$Minus2LogLikelihood
}

PN <- ncol(env[["smr1"]][["dataSummary"]][["fam1"]])*length(env[["smr1"]][["dataSummary"]])
meanDiffLL_mtam_c2p4 <- mean(allMeanDiffLL_c2p4)
...
allMeanDiffLL_c2p5 <- c()
for (i in 1:50){
      env <- new.env()
      load(paste0(relativePath,"/c2p5/","loop",i,"/modelSmr.Rdata"), envir = env)
      allMeanDiffLL_c2p5[i] <- env$smr2$Minus2LogLikelihood - env$smr1$Minus2LogLikelihood
}

PN <- ncol(env[["smr1"]][["dataSummary"]][["fam1"]])*length(env[["smr1"]][["dataSummary"]])
meanDiffLL_mtam_c2p5 <- mean(allMeanDiffLL_c2p5)
