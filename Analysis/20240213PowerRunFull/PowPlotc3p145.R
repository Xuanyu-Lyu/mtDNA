...
env_c3p1 <- new.env()
load(paste0(relativePath,"/","c3p1/modelSmr.Rdata"), envir = env_c3p1)
...
powVec_c3p1 <- as.numeric(lapply(LamdaVec_c3p1,powerCal, df = 2))
...
### create a new enviroment for one condition
env_c3p4 <- new.env()
load(paste0(relativePath,"/","c3p4/modelSmr.Rdata"), envir = env_c3p4)
...
powVec_c3p4 <- as.numeric(lapply(LamdaVec_c3p4,powerCal, df = 2))
...
### create a new enviroment for one condition
env_c3p5 <- new.env()
load(paste0(relativePath,"/","c3p5/modelSmr.Rdata"), envir = env_c3p5)


meanDiffLL_mtam_c3p5 <- env_c3p5$smr2$Minus2LogLikelihood - env_c3p5$smr1$Minus2LogLikelihood
PN <- ncol(env_c3p5[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c3p5[["smr1"]][["dataSummary"]])
lamdaUnit_c3p5 <- meanDiffLL_mtam_c3p5/PN
SSize_c3p5 <- 1: n
LamdaVec_c3p5 <- lamdaUnit_c3p5*SSize_c3p5
powVec_c3p5 <- as.numeric(lapply(LamdaVec_c3p5,powerCal, df = 2))

df_c3p5 <- data.frame(Nped = SSize_c3p5, 
                      power = powVec_c3p5, 
...
# SSize_c3p5 <- 1: n
# LamdaVec_c3p5 <- lamdaUnit_c3p5*SSize_c3p5
# powVec_c3p5 <- as.numeric(lapply(LamdaVec_c3p5,powerCal, df = 1))
...
df_c3 <- rbind(df_c3p1, df_c3p4, df_c3p5
               #,df_c3p4,df_c3p5
               )
...
ggsave( paste0(relativePath_save,"/graph_c3p145.png"), g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
