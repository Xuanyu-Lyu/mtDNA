...
env_c1p1 <- new.env()
load(paste0(relativePath,"/","c1p1/modelSmr.Rdata"), envir = env_c1p1)
...
powVec_c1p1 <- as.numeric(lapply(LamdaVec_c1p1,powerCal, df = 2))
...
### create a new enviroment for one condition
env_c1p4 <- new.env()
load(paste0(relativePath,"/","c1p4/modelSmr.Rdata"), envir = env_c1p4)
...
powVec_c1p4 <- as.numeric(lapply(LamdaVec_c1p4,powerCal, df = 2))
...
powVec_c1p5 <- as.numeric(lapply(LamdaVec_c1p5,powerCal, df = 2))
...
df_c1 <- rbind(df_c1p1, df_c1p4, df_c1p5
               #,df_c1p4,df_c1p5
               )
...
ggsave( paste0(relativePath_save,"/graph_c1p145.png"), g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
